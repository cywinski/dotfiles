...
`assets/tikz-preamble.tex` defines the block, data, arrow, and stage styles
and the same palette the plots use. Copy it as the head of the figure's `.tex`:

```sh
mkdir -p figs && cp ~/.claude/skills/plotting/assets/tikz-preamble.tex figs/method.tex
```

A `.tex` in the working tree compiles where it sits, so `figs/method.tex`
produces `figs/method.pdf` next to it, ready for `\includegraphics`
(`pdflatex -output-directory=figs figs/method.tex`).
...
```latex
% ... contents of tikz-preamble.tex above this line ...
...
`\familydefault` across — the sans face rides in the `fig`, `edgelbl`, and
